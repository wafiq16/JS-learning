...
async function getMultiple(page = 1) {
    ...
    const rows = await db.query(
        'SELECT id, quote, author FROM quote OFFSET $1 LIMIT $2;', [offset, config.listPerPage]
        // 'SELECT * FROM quote;'
    );
    // console.log(rows);
...
async function create(quote) {
    ...
    const result = await db.query(
        'INSERT INTO quote(quote, author) VALUES ($1, $2) RETURNING *', [quote.quote, quote.author]
    );
